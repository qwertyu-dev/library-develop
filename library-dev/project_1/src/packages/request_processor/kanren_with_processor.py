...
# chain詰め込み定義 Pre
## 名称ではなくObjectそのものを格納
class KanrenWithPreProcessor(PreProcessor):
    def chain_pre_process(self) -> list[PreProcessor]:
        return [
            MapperProcessExcelColtoPythonColKanren(),
            MapperProcessToIntegratedLayoutKanrenWith(),
            #DummyPreProcess1(),
            #DummyPreProcess2(),
        ]
...
# chain詰め込み定義 Post
## 名称ではなくObjectそのものを格納
class KanrenWithPostProcessor(PostProcessor):
...
class MapperProcessToIntegratedLayoutKanrenWith(PreProcessor):
    """日本語ExcelColをPython変数名にmapping"""
    def process(self, df: pd.DataFrame) -> pd.DataFrame:
        _df = df.copy()
        column_mapping_kanren_with_dummy = KanrenExcelMappingWithDummy()
        return column_mapping_kanren_with_dummy.map_to_unified_layout(_df)
