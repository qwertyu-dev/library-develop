...
# chain詰め込み定義 Pre
## 名称ではなくObjectそのものを格納
class KanrenWithoutPreProcessor(PreProcessor):
    def chain_pre_process(self) -> list[PreProcessor]:
        return [
            MapperProcessExcelColtoPythonColKanren(),
            MapperProcessToIntegratedLayoutKanrenWithout(),
            #DummyPreProcess1(),
            #DummyPreProcess2(),
        ]
...
# chain詰め込み定義 Post
## 名称ではなくObjectそのものを格納
class KanrenWithoutPostProcessor(PostProcessor):
...
class MapperProcessToIntegratedLayoutKanrenWithout(PreProcessor):
    """日本語ExcelColをPython変数名にmapping"""
    def process(self, df: pd.DataFrame) -> pd.DataFrame:
        _df = df.copy()
        column_mapping_kanren_without_dummy = KanrenExcelMappingWithoutDummy()
        return column_mapping_kanren_without_dummy.map_to_unified_layout(_df)
