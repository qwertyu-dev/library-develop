...
class BranchCodeProcessor:
    ...
    StaticMethods:
        process_branch_codes(df: pd.DataFrame) -> pd.DataFrame: DataFrameから一意な部店コードテーブルを生成する
    ...
    def _preprocess_branch_code(self, code: str) -> str | None:
        ...
        '1234'
        """
    def _preprocess_branch_code(self, code) -> str | None:
        ...
        if len(code_str) == self.BRANCH_PROCESS:
            return code_str
        if len(code_str) == self.SECTION_GR_PROCESS:
            if code_str.startswith((self.BRANCH_7777, self.BRANCH_8888)):
                return code_str
            return code_str[:4]

        error_msg = "Invalid branch code length."
    ...
    def generate_unique_branch_code(df: pd.DataFrame) -> pd.DataFrame:
        ...
        Usage Example:
        >>> df = pd.DataFrame({'branch_code': ['1234', '78201', '77465', '1234', '5678']})
        >>> BranchCodeProcessor.generate_unique_branch_code(df)
            processed_branch_code
        0                   1234
        1                   5678
        2                  77465
        3                  78201
        ...
        if 'branch_code' not in df.columns:
            err_msg = "DataFrame must contain 'branch_code' column"
            raise ValueError(err_msg)

        processor = BranchCodeProcessor()
        processed_codes = df['branch_code'].apply(processor._preprocess_branch_code)

        unique_codes_df = pd.DataFrame({
            'processed_branch_code': processed_codes[processed_codes.notna()].unique(),
        })

        return unique_codes_df.sort_values('processed_branch_code').reset_index(drop=True)
