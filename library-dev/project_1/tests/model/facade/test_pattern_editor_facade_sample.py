...
from src.model.facade.pattern_editor_facade_sample import DataFrameEditorDefault
...
class TestDataFrameEditorDefault:
    ...
    def output_layout(self):
        """出力レイアウトのfixture"""
        return [
            'ulid', 'update_type', 'branch_code_bpr',
            'xxxx', 'xxxy', 'xxxz', 'x2', 'x4',
    ...
    def test_initialize_editors_C0_creation(self):
        ...
        assert isinstance(editors['ulid'], Column1Editor)
        assert isinstance(editors['update_type'], Column2Editor)
        assert isinstance(editors['branch_code_bpr'], Column3Editor)

    @patch('src.model.facade.base_facade.DataFrameEditor.edit_series')
    @patch('src.model.facade.pattern_editor_facade_sample.parse_str_to_datetime')
    def test_edit_series_C0_flow(self, mock_parse_datetime, mock_parent_edit, basic_series, output_layout):
        ...
        mock_parse_datetime.assert_called_once_with('2024/10/25')
    ...
    def test_edit_series_data_basic(self, mock_parse_datetime, basic_series, output_layout):
        ...
        assert result['xxxx'] == '荻野'
        assert result['xxxy'] == '斉藤'
        assert result['xxxz'] == '豊田'
